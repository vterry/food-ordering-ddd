...
func TestMenuAppService_ActiveMenu(t *testing.T) {
	...
	restUUID := uuid.New().String()
	restID, _ := valueobjects.ParseRestaurantId(restUUID)
	menuUUID := uuid.New().String()
	menuID, _ := valueobjects.ParseMenuId(menuUUID)

	menuAgg, _ := menu.NewMenu("Menu Test", restID)
	menuAgg = menu.Restore(menuID, "Menu Teste", restID, enums.MenuDraft, []menu.Category{*category})
